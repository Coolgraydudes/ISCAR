...
export default function IntroUI() {
  ...
  return (
    <div className={`fixed inset-0 z-20 text-white select-none transition-opacity duration-700 ease-out ${exploreClicked ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
      
      {/* --- MOBILE VIEW --- */}
      <div className="lg:hidden absolute inset-0 pointer-events-none">
        <div className="pointer-events-auto absolute top-6 left-6 right-6 flex items-center justify-between">
          <span className="font-bold tracking-tight text-white" style={{ fontFamily: clashDisplay, fontSize: 'clamp(24px, 6vw, 32px)' }}>ISCAR</span>
          <div className="h-9 w-9 rounded-full border border-white/30 flex items-center justify-center">
            <div className="space-y-1">
              {[1,2,3].map(i => <span key={i} className="block w-4 h-px bg-white" />)}
            </div>
          </div>
        </div>

        <div className="absolute top-[40%] left-1/2 -translate-x-1/2 -translate-y-1/2 w-full px-6 flex flex-col items-center text-center gap-6">
          <div className="flex flex-col items-center text-white" style={{ fontFamily: interTightBold }}>
            <span style={{ fontSize: 'clamp(22px, 8vw, 36px)', lineHeight: '1.2' }}>INDEPENDENT</span>
            <span style={{ fontSize: 'clamp(22px, 8vw, 36px)', lineHeight: '1.2' }}>STUDENT COUNCIL</span>
            <span style={{ fontSize: 'clamp(22px, 8vw, 36px)', lineHeight: '1.2' }}>OF ABU DZAR</span>
          </div>
          <button onClick={handleExploreClick} className="pointer-events-auto px-12 py-3 rounded-full border border-white/20 bg-white/5 backdrop-blur-md text-sm text-white" style={{ fontFamily: interSemiBold }}>
            Explore
          </button>
        </div>

        {/* Text yang kamu tanyakan di Mobile menggunakan CLAMP */}
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 w-[85%] text-left leading-relaxed text-white/80" 
             style={{ fontFamily: interTight, fontSize: 'clamp(12px, 3.5vw, 15px)' }}>
      ...
      {/* --- DESKTOP VIEW (Aktif di 1280x720) --- */}
      <div className={`hidden lg:flex fixed inset-0 flex-col justify-between py-10 transition-opacity duration-700 ${exploreClicked ? "opacity-0" : "opacity-100"} ${uiHidden ? "pointer-events-none" : "pointer-events-auto"}`}>
        
        <div className="w-full">
          <div className="relative w-full overflow-hidden top-3 flex justify-center">
             {/* Logo dengan Clamp lebar agar pas di 1280px */}
             <div className="w-full px-[7vw]">
                <svg viewBox="0 0 584 126" className="w-full h-auto max-w-[1400px]" xmlns="http://www.w3.org/2000/svg">
          ...
          <nav className="flex w-full justify-between mt-4 px-[10vw]" style={{ fontFamily: interTight, fontSize: 'clamp(10px, 0.8vw, 14px)' }}>
        ...
        <div className="grid grid-cols-3 items-end px-[10vw] pb-10">
          <div className="flex flex-col text-left" style={{ fontFamily: interTight }}>
            <BlurText text="Independent" delay={900} style={{ fontSize: 'clamp(12px, 1vw, 16px)' }} />
            <BlurText text="Student Council" delay={1000} style={{ fontSize: 'clamp(12px, 1vw, 16px)' }} />
            <BlurText text="Of Abu Dzar" delay={1100} style={{ fontSize: 'clamp(12px, 1vw, 16px)' }} />
          </div>

          <div className="flex justify-center pointer-events-auto">
            <div className="relative -mt-40 transition-transform duration-300 ease-out hover:scale-[1.05] will-change-transform">
              <button onClick={handleExploreClick} onMouseEnter={() => { if (!exploreClicked) setHoverZoom(true) }} onMouseLeave={() => { if (!exploreClicked) setHoverZoom(false) }}
                className="flex h-14 w-52 items-center justify-center rounded-[20px] border border-white/20 bg-white/5 backdrop-blur-md transition-shadow duration-300 hover:shadow-[0_0_40px_rgba(255,255,255,0.15)]"
                style={{ fontFamily: interSemiBold }}
              >
                <span className="text-base tracking-tight text-white/90">
          ...
          <div className="flex justify-end">
            <div className="max-w-[20vw] text-right">
              <BlurText
                ...
                style={{ fontFamily: interTight, fontSize: 'clamp(11px, 0.9vw, 14px)' }}
              />
            </div>
          </div>
        </div>
